...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const existingId: string | null = search.json?.results?.[0]?.id || null;
    ...
    if (prix === undefined || prix === null || Number(prix) <= 0) {
      // Pas de prix : on n'envoie pas le produit. Si un produit existait, on l'archive.
      if (existingId) {
        const del = await hs(`/crm/v3/objects/products/${existingId}`, "DELETE");
    ...
    if (existingId) {
      const upd = await hs(`/crm/v3/objects/products/${existingId}`, "PATCH", { properties: { name, price: String(prix) } });
      if (!upd.ok) {
      ...
      const crea = await hs("/crm/v3/objects/products", "POST", { properties: props });
      if (!crea.ok) {

...
export default function DiagnosticModal({ machine, onClose }: { machine: Machine; onClose: () => void }) {
  ...
  const lignes: [string, string][] = [
    ...
    ["Facture de vente", m.numero_facture ? `${m.numero_facture} (${m.date_facturation || "—"})` : "—"],
    ["Type de préparation", m.type_prepa || "—"],
    ["Étapes prépa", Array.isArray(m.etapes_prepa) ? `${m.etapes_prepa.filter((e: any) => e.done || e.non_necessaire).length}/${m.etapes_prepa.length}` : "—"],
    ["HubSpot", m.hubspot_synced ? "✅ deal synchronisé" : "—"],
    ["⚠️ Alerte saisie", m.alerte_saisie || "—"],

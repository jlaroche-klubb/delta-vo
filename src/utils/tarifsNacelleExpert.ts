// ============================================================
// 💶 COPIE du barème Nacelle Expert (api/_tarifs-defaults.js)
// ============================================================
// Utilisée UNIQUEMENT par l'outil super admin « chiffrage à zéro »
// (recalcul des montants d'expertise pour les anciens dossiers dont
// les postes « sur devis » n'ont jamais été chiffrés).
// ⚠ Copie ponctuelle : si les tarifs évoluent côté Nacelle Expert,
// ce fichier n'est pas mis à jour automatiquement — acceptable pour
// une opération de rattrapage qui ne se reproduira plus.
// Règle appliquée (validée avec Jonathan) : un poste « sur devis »
// HORS zone nacelle prend la 1re tranche de son barème ; un poste
// nacelle sans chiffrage atelier reste à 0 (« en attente de devis »).
// ============================================================

/* eslint-disable */
export interface TarifNE {
  id: string; zone: string; label: string; prix: number | null;
  surDevis?: boolean; bareme?: { label: string; montant: number }[];
}

export const DEFAULT_TARIFS: TarifNE[] = [
  // CARROSSERIE
...
export function getVetuste(annee_fab: any): number {
...
export const prixAvecVetuste = (prix: any, taux: number): number => (prix ? Math.round(prix * (1 + taux / 100)) : 0);
...
/**
 * Recalcule le chiffrage d'un dossier Nacelle Expert (lu directement dans
 * la base NE, lecture publique) avec la règle actuelle :
 * - poste au tarif fixe → prix × quantité, vétusté appliquée
 * - poste sur devis déjà chiffré (montants_devis / devis_recu) → montant connu
 * - poste sur devis HORS nacelle non chiffré → 1re tranche du barème
 * - poste sur devis NACELLE non chiffré → 0, compté « en attente de devis »
 */
export function calculerChiffrageDossier(d: any): {
  ...
  const ids: string[] = Array.isArray(retour.degats) ? retour.degats : [];
  ...
  const degats = ids.map((id) => {
    const t = DEFAULT_TARIFS.find((x) => x.id === id) || null;
    ...
    if (surDevis) {
      if (recu[id] && recu[id].inclus) {
        description += " — inclus au devis atelier";
      } else {
        montant = Number(md[id]) || Number(recu[id] && recu[id].montant) || 0;
        if (!montant) {
          if (t && t.zone !== "nacelle" && Array.isArray(t.bareme) && t.bareme.length && Number(t.bareme[0].montant) > 0) {
            montant = Number(t.bareme[0].montant);
            description += ` (${t.bareme[0].label})`;
          } else {
            nbAttente++;
            description += " — en attente de devis";
          }
        }
      }
    } else {
